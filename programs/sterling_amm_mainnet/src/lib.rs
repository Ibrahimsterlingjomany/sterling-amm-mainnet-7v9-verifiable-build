...
security_txt! {
    name: "Sterling DEX",
    project_url: "https://github.com/Ibrahimsterlingjomany/sterling-amm-mainnet-7v9-verifiable-build",
    contacts: "link:https://github.com/Ibrahimsterlingjomany/sterling-amm-mainnet-7v9-verifiable-build/security/advisories/new",
    policy: "https://github.com/Ibrahimsterlingjomany/sterling-amm-mainnet-7v9-verifiable-build/blob/main/SECURITY.md",
    preferred_languages: "fr,en",
    auditors: "No independent third-party audit publicly declared"
}
...
pub const METAPLEX_TOKEN_METADATA_PROGRAM_ID: &str =
...
pub mod sterling_amm {
    ...
    pub fn init_protocol_debt_ledger(ctx: Context<InitProtocolDebtLedger>) -> Result<()> {
        ...
        ensure_protocol_debt_ledger_account(
            &ctx.accounts.protocol_debt_ledger.to_account_info(),
            &ctx.accounts.admin,
            &ctx.accounts.system_program,
            ctx.accounts.pool.key(),
            ctx.bumps.protocol_debt_ledger,
        )?;
        initialize_protocol_debt_ledger_account(
            &ctx.accounts.protocol_debt_ledger.to_account_info(),
            ctx.accounts.pool.key(),
            ctx.bumps.protocol_debt_ledger,
        )?;
    ...
    pub fn upsert_pool_lp_metadata(
        ctx: Context<UpsertPoolLpMetadata>,
        name: String,
        symbol: String,
        uri: String,
    ) -> Result<()> {
        ...
        crate::helpers::require_admin(&config, &ctx.accounts.admin)?;
        let (expected_config, config_bump) =
            Pubkey::find_program_address(&[b"config"], ctx.program_id);
        require_keys_eq!(config_key, expected_config, SterlingError::InvalidAccount);

        let pool = load_pool_snapshot(&pool_info)?;
        let (expected_pool, _) = Pubkey::find_program_address(
            &[b"pool", pool.base_mint.as_ref(), pool.quote_mint.as_ref()],
            ctx.program_id,
        );
        require_keys_eq!(pool_key, expected_pool, SterlingError::InvalidAccount);

        let lp_mint = load_mint_snapshot(&lp_mint_info)?;
        let (expected_lp_mint, _) =
            Pubkey::find_program_address(&[b"lp_mint", pool_key.as_ref()], ctx.program_id);
        require_keys_eq!(lp_mint_key, expected_lp_mint, SterlingError::InvalidAccount);
        require!(
            !name.is_empty() && !symbol.is_empty() && !uri.is_empty(),
            SterlingError::InvalidMetadataFieldLength
        );
        require!(
            name.as_bytes().len() <= 32
                && symbol.as_bytes().len() <= 10
                && uri.as_bytes().len() <= 200,
            SterlingError::InvalidMetadataFieldLength
        );
        require!(
        ...
        require!(
            ...
            SterlingError::InvalidLpMintAuthority
        );

        let token_metadata_program_id = pubkey_from_str(METAPLEX_TOKEN_METADATA_PROGRAM_ID);
        ...
        require_keys_eq!(
            ...
            expected_metadata,
            SterlingError::InvalidMetadataAccount
        );
        require_keys_eq!(
        ...
        let signer_seeds: &[&[&[u8]]] = &[&[b"config", &config_bump_bytes]];
        ...
        if ctx.accounts.metadata.data_is_empty() {
            let instruction = Instruction {
                ...
                accounts: vec![
                    ...
                    AccountMeta::new_readonly(lp_mint_key, false),
                    AccountMeta::new_readonly(config_key, true),
                    AccountMeta::new(ctx.accounts.admin.key(), true),
                    AccountMeta::new_readonly(config_key, true),
                    AccountMeta::new_readonly(ctx.accounts.system_program.key(), false),
                ],
                data: build_create_metadata_account_v3_data(&name, &symbol, &uri),
            };
            invoke_signed(
                &instruction,
                &[
            ...
            let instruction = Instruction {
                ...
                data: build_update_metadata_account_v2_data(&name, &symbol, &uri),
            };
            invoke_signed(
                &instruction,
                &[
    ...
    pub fn convert_fees_to_usdc_fragmented(
        ...
        require!(target_usdc_out > 0, SterlingError::InvalidAmount);

        let (source_vault, source_key) = match fee_side {
            FeeSide::Base => {
                require!(
                    ctx.accounts.fee_mint.key() == ctx.accounts.pool.base_mint,
                    SterlingError::InvalidAccount
                );
                (
                    &ctx.accounts.fee_vault_base,
            ...
            FeeSide::Quote => {
                require!(
                    ctx.accounts.fee_mint.key() == ctx.accounts.pool.quote_mint,
                    SterlingError::InvalidAccount
                );
                (
                    &ctx.accounts.fee_vault_quote,
        ...
        if ctx.accounts.fee_mint.key() == ctx.accounts.config.usdc_mint {
            require!(
                source_vault.amount >= target_usdc_out,
                SterlingError::InsufficientFeeVaultBalance
            );
            transfer_tokens_signed(
                &ctx.accounts.token_program,
        ...
        require!(
            source_vault.amount >= burn_amount,
            SterlingError::InsufficientFeeVaultBalance
        );
        require!(
            ctx.accounts.usdc_coffre_ata.amount >= target_usdc_out,
            SterlingError::InsufficientUsdcSettlementLiquidity
        );
...
#[derive(Accounts)]
pub struct UpsertPoolLpMetadata<'info> {
    /// CHECK: Config PDA, validated in handler.
    pub config: UncheckedAccount<'info>,
    pub admin: Signer<'info>,
    /// CHECK: Pool PDA, validated in handler.
    pub pool: UncheckedAccount<'info>,
    /// CHECK: LP mint PDA, validated in handler.
    pub lp_mint: UncheckedAccount<'info>,
    /// CHECK: Metaplex metadata PDA for the LP mint, validated in handler.
    #[account(mut)]
    pub metadata: UncheckedAccount<'info>,
    /// CHECK: Must be the Metaplex metadata program.
...
pub enum SterlingError {
    ...
    #[msg("Insufficient fee vault balance for requested conversion")]
    ...
    #[msg("Metadata field exceeds Metaplex limits")]
    InvalidMetadataFieldLength,
}
...
fn build_metaplex_data_v2(name: &str, symbol: &str, uri: &str) -> Vec<u8> {
    let mut data = Vec::with_capacity(
        4 + name.as_bytes().len() + 4 + symbol.as_bytes().len() + 4 + uri.as_bytes().len() + 5,
    );
...
fn build_create_metadata_account_v3_data(name: &str, symbol: &str, uri: &str) -> Vec<u8> {
    let data_v2 = build_metaplex_data_v2(name, symbol, uri);
    let mut instruction_data = Vec::with_capacity(1 + data_v2.len() + 2);
    instruction_data.push(33);
    instruction_data.extend_from_slice(&data_v2);
    instruction_data.push(1);
    instruction_data.push(0);
    instruction_data
}

fn build_update_metadata_account_v2_data(name: &str, symbol: &str, uri: &str) -> Vec<u8> {
    let data_v2 = build_metaplex_data_v2(name, symbol, uri);
    let mut instruction_data = Vec::with_capacity(1 + 1 + data_v2.len() + 4);
...
fn compute_fee_amount_for_target_usdc_out_ceil(
    target_usdc_out: u64,
    valuation_usd_micros: u64,
    fee_decimals: u8,
) -> Result<u64> {
    ...
    let scale = pow10_u128(fee_decimals);
    require!(scale > 0, SterlingError::MathOverflow);
    let numerator = (target_usdc_out as u128).saturating_mul(scale);
    let denominator = valuation_usd_micros as u128;
    require!(denominator > 0, SterlingError::MathOverflow);

    let ceil_amount = numerator
        .checked_add(denominator.saturating_sub(1))
        .ok_or(SterlingError::MathOverflow)?
        .checked_div(denominator)
        .ok_or(SterlingError::MathOverflow)?;
    require!(ceil_amount > 0, SterlingError::ZeroUsdcSettlement);

    Ok(u64::try_from(ceil_amount).map_err(|_| SterlingError::MathOverflow)?)
}
...
mod tests {
    ...
    #[test]
    fn compute_fee_amount_for_target_usdc_out_ceil_minimal_fragment() {
        let burn_amount =
            compute_fee_amount_for_target_usdc_out_ceil(1_000_000, 174_000_000_000, 9).unwrap();
        assert_eq!(burn_amount, 5_748);
        let produced = compute_usdc_settlement_amount(burn_amount, 174_000_000_000, 9).unwrap();
        assert!(produced >= 1_000_000);
    }

    #[test]
    fn compute_fee_amount_for_target_usdc_out_ceil_exact_usdc_identity() {
        let burn_amount =
            compute_fee_amount_for_target_usdc_out_ceil(1_000_000, 1_000_000, 6).unwrap();
    ...
    #[test]
    fn initialize_protocol_debt_ledger_sets_header_only() {
        let pool = Pubkey::new_unique();
        let mut ledger = ProtocolDebtLedger {
            pool: Pubkey::default(),
            next_nonce: 0,
            overflow_usd_micros: 99,
            last_ts: 123,
            bump: 0,
            lots: [ProtocolDebtLot::default(); PROTOCOL_DEBT_LEDGER_SLOTS],
        };

        initialize_protocol_debt_ledger(&mut ledger, pool, 7);

        assert_eq!(ledger.pool, pool);
        assert_eq!(ledger.next_nonce, 1);
        assert_eq!(ledger.overflow_usd_micros, 0);
        assert_eq!(ledger.last_ts, 0);
        assert_eq!(ledger.bump, 7);
        assert!(ledger
            .lots
            .iter()
            .all(|lot| *lot == ProtocolDebtLot::default()));
    }
...
fn initialize_protocol_debt_ledger(
    ledger: &mut ProtocolDebtLedger,
    pool: Pubkey,
    bump: u8,
) {
    ...
    ledger.last_ts = 0;
    ledger.bump = bump;
    // `init` creates a brand new zeroed account, so the lots array is already empty.
    // Avoid rebuilding the whole fixed array on stack here: the live binary was
    // tripping an access violation inside `initProtocolDebtLedger`.
}

fn ensure_protocol_debt_ledger_account<'info>(
    ledger_info: &AccountInfo<'info>,
    payer: &Signer<'info>,
    system_program_account: &Program<'info, System>,
    pool: Pubkey,
    bump: u8,
    ...
    if *ledger_info.owner == crate::ID {
        require!(
            ledger_info.data_len() >= ProtocolDebtLedger::LEN,
            SterlingError::InvalidAccount
        );
        return Ok(());
    }
    ...
    require!(
        *ledger_info.owner == anchor_lang::system_program::ID
            && ledger_info.data_is_empty()
            && ledger_info.lamports() == 0,
        SterlingError::InvalidAccount
    );

    let rent = Rent::get()?;
    let lamports = rent.minimum_balance(ProtocolDebtLedger::LEN);
    let bump_bytes = [bump];
    let pool_bytes = pool.to_bytes();
    let signer_seed_slice: &[&[u8]] = &[b"protocol_debt", pool_bytes.as_ref(), &bump_bytes];
...
fn initialize_protocol_debt_ledger_account<'info>(
    ledger_info: &AccountInfo<'info>,
    pool: Pubkey,
    bump: u8,
) -> Result<()> {
    require_keys_eq!(*ledger_info.owner, crate::ID, SterlingError::InvalidAccount);

    let discriminator = ProtocolDebtLedger::discriminator();
    let pool_bytes = pool.to_bytes();

    {
        ...
        if data[..8] == discriminator && data[8..40] == pool_bytes {
            return Ok(());
        }

        let header_is_empty = data[..8].iter().all(|byte| *byte == 0)
            && data[8..40].iter().all(|byte| *byte == 0);
        require!(header_is_empty, SterlingError::InvalidAccount);
    }

    let mut data = ledger_info.try_borrow_mut_data()?;
    data.fill(0);
    data[..8].copy_from_slice(&discriminator);
    data[8..40].copy_from_slice(&pool_bytes);
...
pub struct InitProtocolDebtLedger<'info> {
    ...
    pub pool: Box<Account<'info, Pool>>,
    /// CHECK:
    /// This PDA is created manually to avoid the Anchor `init` path blowing the
    /// stack when materializing the large fixed-size debt ledger account.
    #[account(mut, seeds = [b"protocol_debt", pool.key().as_ref()], bump)]
    pub protocol_debt_ledger: UncheckedAccount<'info>,
    pub system_program: Program<'info, System>,
}
